    ...
    polling_command[3] = StartAddress & 0xFF;
     ...
     switch(StartAddress){
        ...
        case MOD_CALIBRATION_POINT:
            polling_command[4] = UI_Calibration_Point & 0xFF;        // Low byte
            polling_command[5] = (UI_Calibration_Point >> 8) & 0xFF; // High byte
        break;
        case MOD_CALI_RESULT:
            polling_command[4] = 0xFF;       // Low byte
            polling_command[5] = 0xFF;       // High byte
        break;
        default:
        break;
    }
...
int Modbus_RxProcess(uint8_t *response, size_t max_response_size)
{
    ...
    if (bytes_read > 2) { // CRC 2 bytes
        ...
        if ((received_crc == calculated_crc) && (response[1] == 0x03)) {
            ...
            if(Mod_receive_data_error_cnt >= 10)
            {
